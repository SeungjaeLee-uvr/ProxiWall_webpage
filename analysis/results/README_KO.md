...
- 각 사용자: `Ours` 3회 + `Baseline` 3회
- 사용자 1·3·5는 Ours 먼저, 사용자 2·4·6은 Baseline 먼저
...
| 평균 Task Completion Time | 24.80초 | 42.02초 | Ours가 평균 17.22초 빠름 |
| 평균 활성 시간 | 54.74초 | 87.61초 | Ours가 평균 32.87초 짧음 |
| 첫 응답까지 시간 | 24.79초 | 37.68초 | Ours가 평균 12.90초 빠름 |
| 두 응답 사이 시간 | 24.82초 | 46.37초 | Ours가 평균 21.55초 빠름 |
| 후보 변경 수 | 21.39회 | 13.28회 | Ours에서 탐색 후보 전환이 많음 |
| 탐색 방향 반전 수 | 8.22회 | 3.50회 | Ours에서 왕복 탐색이 많음 |
...
| Wrong Touch rate | 51.0% | 85.8% | Baseline의 wrong-touch 비율이 높음 |
| Wrong Selection Touch/session | 1.28회 | 4.39회 | 유물 선택 이벤트만 보아도 Baseline가 많음 |
...
정의된 Wrong Touch 규칙에서 총 `touch_started` 483회 중 Ours는 65회, Baseline는 346회가 wrong으로 분류되었다. 세션별 사용자 paired test에서도 Ours의 Wrong Touch 수가 유의하게 적었다(`p=.0215`, `dz=-1.35`). 터치 수 차이의 영향을 일부 통제한 Wrong Touch rate도 Ours가 낮았다(`p=.0056`, `dz=-1.90`).
...
별도 UI 상태 지표로, `Ours`의 유효 selected 응답 35개 중 24개가 candidate와 일치했고 `Baseline`는 유효 selected 응답 32개 모두 일치했다. 이는 정답률이 아니라 사용자가 정답을 말한 순간 후보와 확정 선택이 같은지를 나타낸다.
